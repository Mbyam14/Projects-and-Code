...
module TSOS {
    export class Shell {
        ...
        public init() {
            ...
            // Hogwarts House
            sc = new ShellCommand(this.shellMyhouse,
                                    "myhouse",
                                    "- Determines your Hogwarts House.");
            this.commandList[this.commandList.length] = sc;

            // Load
            sc = new ShellCommand(this.shellLoad,
                                    "load",
                                    "- Load and Verify user entered program.");
            this.commandList[this.commandList.length] = sc;

             // BSOD
             sc = new ShellCommand(this.shellBsod,
                                    "bsod",
                                    "- Displays the infamous Blue Screen of Death.");
            this.commandList[this.commandList.length] = sc;
            ...
            sc = new ShellCommand(this.shellRun,
                                    "run",
                                    "<pid> - Runs a specified program in memory.");
            this.commandList[this.commandList.length] = sc;
            ...
            sc = new ShellCommand(this.shellKill,
                                    "kill",
                                    "<pid> - Kills process of specified <pid>.");
            this.commandList[this.commandList.length] = sc;
            ...
            // FCFS
            sc = new ShellCommand(this.shellFCFS,
                                    "fcfs",
                                    "- Sets the scheduler to First Come First Serve.");
            this.commandList[this.commandList.length] = sc;


            // Round Robin
            sc = new ShellCommand(this.shellRoundRobin,
                                    "rr",
                                    "- Sets the scheduler to Round Robin with quantum of 6.");
            this.commandList[this.commandList.length] = sc;
            ...
            sc = new ShellCommand(this.shellFormat,
                                    "format",
                                    "- Initalize all blocks in all sectors in all tracks.");
            this.commandList[this.commandList.length] = sc;

            // create <fn>
            sc = new ShellCommand(this.shellCreate,
                                    "create",
                                    "<fn> - Create the file <fn>.");
            this.commandList[this.commandList.length] = sc;

            // read <fn>
            sc = new ShellCommand(this.shellRead,
                                    "read",
                                    "<fn> - Read and display the contents of <fn>.");
            this.commandList[this.commandList.length] = sc;

            // write <fn> "data"
            sc = new ShellCommand(this.shellWrite,
                                    "write",
                                    "<fn> \"data\" - Write the data inside the quotes to <fn>.");
            this.commandList[this.commandList.length] = sc;

            // delete <fn>
            sc = new ShellCommand(this.shellDelete,
                                    "delete",
                                    "<fn> - Remove <fn> from storage.");
            this.commandList[this.commandList.length] = sc;

            // copy <existing fn> <new fn>
            sc = new ShellCommand(this.shellCopy,
                                    "copy",
                                    "<existing fn> <new fn> - Copys data from <existing fn> to <new fn>.");
            this.commandList[this.commandList.length] = sc;

            // rename <existing fn> <new fn>
            sc = new ShellCommand(this.shellRename,
                                    "rename",
                                    "<existing fn> <new fn> - Renames the <existing fn> to <new fn>.");
            this.commandList[this.commandList.length] = sc;
        ...
        public shellMyhouse(args: string[]) {
            var Houses:string[];
        ...
        public shellLoad(args: string[]) {
            var input = (<HTMLInputElement>document.getElementById("taProgramInput")).value.trim();
        ...
        public shellStatus(args: string[]) {
            if (args.length > 0) {
                (<HTMLInputElement> document.getElementById("status")).innerText = "Status: " + args.join(' '); 
            } else {
                _StdOut.putText("Usage: prompt <string> Please supply a string.");
            }
        }

        public extractValue(arr, prop) {
            let extractedValue = [];
        ...
        // sets scheduler to use a FCFS algorithm
        public shellRoundRobin(args: string[]) {
